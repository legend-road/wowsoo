  ...
  bosses: [
    {
      ...
      positioning:
        "Boss 固定中央。25H 常见：主坦约在场地一侧（示意 12 点），大团与治疗在对侧半弧散开，保证治疗覆盖且锥形打不进人群。副坦贴主坦侧便于换坦绕行（不能穿 Boss）。分裂保持外环扇区覆盖全场软；踩池人短进中心，其余勿贴池。",
      roles: {
    ...
    },
    {
      ...
      summary:
        "石蹄洛克、软足何、慈心孙三人不共享血量。各自在 66% / 33% 进「绝命措施」刷化身（Boss 身上 DoT 仍会跳）。常见控血绝命序：洛克 → 孙 → 何，禁止重叠。斩杀三人几乎同时到 1%，否则「金莲之缚」回约 30% 血。英雄：酒桶加速；洛克化身共享血可 cleave；衰弱约 80%/4 分钟；灾祸伤害递增（每次约 +10%）。",
      skills: [
        {
        ...
        {
          name: "何：剧毒 / 锁喉 / 凿击",
          desc: "近战留毒池，躲开。锁喉流血持续到何进绝命才清除（奶骑保护之手可解）。凿击：正面吃到瘫痪并锁定随机玩家；何坦读条结束前背对即可躲开（只被短距击退）。双坦：坦 A 洛克、坦 B 何——何坦优先转身，洛克坦兜底嘲讽再还仇。",
          who: "坦克 / 治疗",
        ...
        {
          name: "孙·绝命：黑暗冥想",
          desc: "刷出绝望类化身，孙持续跳暗影伤。全员进中央「冥想力场」减伤并清化身；洛克尽量拉在罩外，远程在罩内清怪时注意躲开洛克对撞踢。",
          who: "全团",
          video: "https://assets2.mythictrap.com/videos/siege-of-orgrimmar/fallen-protectors/darkMeditation.mp4",
        },
        {
          name: "【英雄】机制加压",
          desc: "酒桶飞行随战斗加速。洛克绝命化身共享血，可 cleave。衰弱约 80% 减甲、持续约 4 分钟——印记短持强减。灾祸递增，后期必开大减伤。",
          who: "全团",
        },
      ],
      mechanics: [
        "绝命序示例：洛克 66% → 孙 66% → 何 66%（清锁喉）→ 同样做 33% → 三线压平同步斩杀。",
        "洛克绝命：哀伤炼狱分摊或独吃减伤；阴郁腐蚀震击轮流打断；悲苦拉外放永久黑圈；英雄化身可顺劈。",
        "孙绝命：全员进罩；洛克留罩外；打断/清化身。平时祸秒驱，灾祸预减伤。",
        "何绝命：非坦传递链短持；约 4–5 层减伤，末秒传递；英雄衰弱更狠禁乱传。",
        "斩杀：三人同时到 1%，禁止垫刀不同步。",
      ],
      positioning:
        "轻度散开。洛克背对大团；何外拉毒池；孙靠近便于打断。远程勿站在洛克与其他远程之间挡对撞路径。孙绝命进中央罩；哀伤短暂集合；悲苦黑圈靠边放。",
      roles: {
        tank: "双坦分洛克与何。洛克锥形背离+减伤；凿击何坦背对，洛克坦兜底嘲讽；悲苦拉外放圈；孙绝命洛克尽量罩外。",
        healer: "洛克坦复仇打击重点抬；锁喉/印记点抬；祸秒驱；灾祸与孙绝命开减伤。奶骑保护可解锁喉；戒律灾祸预盾、集合神圣之星。",
    ...
    },
    {
      ...
      summary: "九名英杰，场上通常同时只有三只激活；击杀一只后下一位入场。普通：击杀时其余激活英杰回满并获「英杰之志」约 +8% 伤害。英雄：伤增改为约每 50 秒全局叠层，拖时间更危险。禁止无规划 cleave。推荐击杀序：斯卡尔 → 里卡尔 → 柯尔文 → 希赛克 → 夏克里尔 → 伊约库克 → 卡兹提克 → 克尔鲁克 → 卡罗兹。尸体可捡一次英杰之力（按职责）。",
      skills: [
        {
          name: "英杰之志 / 就绪开战",
